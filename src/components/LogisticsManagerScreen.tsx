...
const LogisticsManagerScreen = () => {
  ...
  const handleHandover = (orderId: string) => {
    ...
    if (window.confirm(`Handover order to ${courierName}?`)) {
        const confirmHandover = async () => {
            try {
              ...
              fetchData();
              alert('Order handed over successfully');
            } catch (error: any) {
            ...
            {isHandedOver && (
              <TouchableOpacity style={styles.transitBtn} onPress={async () => {
                try {
                  ...
                  fetchData();
                  alert('Order is now IN-TRANSIT');
                } catch (error: any) {
                  alert(error.message || 'Failed to update to transit');
                }
              }}>
...
const styles = StyleSheet.create({
  ...
  tabWrapper: { paddingHorizontal: 15, paddingBottom: 10 },
  tabBar: {
    flexDirection: 'row',

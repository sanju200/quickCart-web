...
const InventoryManagerScreen = () => {
  ...
  const handleProcessOrder = async (orderId: string, currentStatus: OrderStatus) => {
    ...
    if (window.confirm('Move this order to PROCESSING status and decrease inventory?')) {
        try {
          ...
          fetchData();
          showToast('Order status updated to PROCESSING and stock adjusted', 'success');
        } catch (error: any) {
          showToast(error.message || 'Failed to update order and stock', 'error');
        }
    }
  };
  ...
  const handleUpdateStock = async (productId: string, delta: number) => {
    ...
    try {
      ...
      showToast(error.message || 'Failed to update stock in database', 'error');
      fetchData();
    }
  };
...
const styles = StyleSheet.create({
  ...
  tabWrapper: { width: '100%', paddingHorizontal: 15, paddingBottom: 10 },
  tabBar: {
    flexDirection: 'row',
  ...
  listWrapper: { width: '100%', padding: 15, paddingBottom: 120 },
  orderCard: {
    backgroundColor: '#fff',
    borderRadius: 16,
    padding: 15,

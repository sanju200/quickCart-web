...
const DeliveryPartnerScreen = () => {
  ...
  const fetchDeliveryOrders = async () => {
    try {
      ...
      if (activeTab === 'ACTIVE') {
        ...
        const totalEarnings = historyOrders.reduce((sum, o) => sum + (Number(o.totalAmount) * 0.1), 0);
  ...
  const handleDeliveryComplete = (orderId: string) => {
    if (!podText) {
      alert('Please provide Proof of Delivery (e.g. Received by [Name])');
      return;
    }

    if (window.confirm('Confirm this order as successfully delivered?')) {
        const completeDelivery = async () => {
            try {
              await updateOrderStatus(orderId, 'delivered', { proofOfDelivery: podText });
              fetchDeliveryOrders();
              alert('Final notification sent to user.');
            } catch (error: any) {
